...
>
> **UPDATE 2026-07-25 — real content run found the Grammar Check was too strict, fixed.** During Phase 6's real multi-tenancy verification, a real pipeline run legitimately hit the 3-retry cap and returned `status: failed` with `"script exceeded 3 retries"`. The script itself was grammatically fine — the Grammar Check's actual flagged "issues" were word-choice consistency ("released" vs "introduced to the market") and a passive-voice preference, neither of which is a grammar error. The original prompt said "spelling, tense, and syntax errors ONLY (not tone, not facts)" but didn't explicitly rule out style/word-choice/voice preferences, and the LLM was reading "syntax" broadly enough to fail correct writing over them. Tightened `grammar_check_prompt()` in `prompt_library.py` with an explicit "do NOT fail for" list (word choice, active/passive voice, sentence rhythm) and permission to pass text "even if you personally would have written it differently." Not yet re-verified against a real run — the faked test suite (which uses a canned Grammar Check response, not this prompt's actual behavior) can't confirm the fix, only that nothing else regressed. Worth watching on the next real run whether this was fully fixed or just made less likely.
>
